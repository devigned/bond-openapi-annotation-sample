...
var swaggerSchemaUrl = "http://swagger.io/v2/schema.json";
...
describe('Azure Swagger Schema Validation', function() {
  before(function(done) {
    request({url: swaggerSchemaUrl, json:true}, function (error, response, swaggerSchemaBody) {
  ...
  _(swaggers).each(function(swagger){
    it(swagger + ' should be valid Swagger', function(done){
      fs.readFile(swagger, 'utf8', function(err, data){
        ...
        var valid = validator.validate(JSON.parse(stripBOM(data)), swaggerSchema);
        if (!valid) {
